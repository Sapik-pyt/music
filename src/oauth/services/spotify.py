...
def get_spotify_email(code):
    _token = get_spotify_jwt(code)
    if _token is not None:
        return get_spotifu_user(_token)
    else:
        return None

def spotify_auth(code):
    email = get_spotify_jwt(code)
    if email is not None:
        user, _ = AuthUser.objects.get_or_create(email=email)
        base_auth.create_token(user.id)
    else:
